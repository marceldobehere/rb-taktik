...
function initApp(_app, _io, _notificationSystem, _friendInterface, _accountInterface, _sessionSystem)
{
    ...
    io.on('connection', (socket) => {
        socket.on('challenge-user', async (obj) => {
            let sessionObj = sessionSystem.getSessionBySocket(socket);
            if (sessionObj === undefined)
                return socket.emit('challenge-user', {error: "Invalid session"});

            let userId1 = sessionObj.userId;

            let challengeUserId = obj["userId"];
            if (challengeUserId === undefined)
                return socket.emit('challenge-user', {error: "Missing challenge user id"});

            let roomId = obj["roomId"];
            if (roomId === undefined)
                return socket.emit('challenge-user', {error: "Missing room id"});

            if (await accountInterface.getUser(challengeUserId) === undefined)
                return socket.emit('challenge-user', {error: "Invalid user id"});

            if (! await friendInterface.areFriends(userId1, challengeUserId))
                return socket.emit('challenge-user', {error: "Not friends"});

            if (! await notificationSystem.sendMatchRequest(userId1, challengeUserId, roomId))
                return socket.emit('challenge-user', {error: "Challenge request sending failed"});

            socket.emit('challenge-user', {});
        });

        socket.on('decline-challenge', async (obj) => {
            let sessionObj = sessionSystem.getSessionBySocket(socket);
            if (sessionObj === undefined)
                return socket.emit('decline-challenge', {error: "Invalid session"});

            let userId1 = sessionObj.userId;

            let challengeUserId = obj["userId"];
            if (challengeUserId === undefined)
                return socket.emit('decline-challenge', {error: "Missing challenge user id"});

            let roomId = obj["roomId"];
            if (roomId === undefined)
                return socket.emit('decline-challenge', {error: "Missing room id"});

            if (await accountInterface.getUser(challengeUserId) === undefined)
                return socket.emit('decline-challenge', {error: "Invalid user id"});


            if (! await friendInterface.areFriends(userId1, challengeUserId))
                return socket.emit('decline-challenge', {error: "Not friends"});

            // Maybe send challenge declined msg
            //if (! await notificationSystem.sendMatchRequest(userId1, challengeUserId, roomId))
            //    return socket.emit('challenge-user', {error: "Challenge request sending failed"});

            let otherSession = sessionSystem.getSessionByUserId(challengeUserId);
            if (otherSession === undefined)
                return socket.emit('decline-challenge', {error: "User not avaiable"});
            let otherSocket = otherSession.socket;
            if (otherSocket === undefined)
                return socket.emit('decline-challenge', {error: "User not avaiable"});

            otherSocket.emit('decline-challenge', {userId: userId1, roomId: roomId});
    ...
    console.log("> Initialized challenge system");
}
